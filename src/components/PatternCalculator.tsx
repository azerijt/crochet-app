...
export default function PatternCalculator(props: StitchSizeObject): JSX.Element {
    ...
    function onPatternClick(){
        ...
        setPatternSize({stitchName: '', stitchWidth: parseInt(patternWidth)/props.stitchWidth, stitchHeight: parseInt(patternHeight)/props.stitchHeight});
    }
    ...
    return(
        <div> 
            ...
            value={patternWidth}></input>
            <p> Enter desired pattern height CM: </p>

            <input
            placeholder ="Desired height CM"
            onChange={(e)=>setPatternHeight(e.target.value)}
            value={patternHeight}></input>
            <button onClick={onPatternClick}>Calculate Pattern</button>
            {patternSize? <p> Pattern Width: {patternSize.stitchWidth}chains x Pattern Height: {patternSize.stitchHeight} rows</p>: null}
